...
function AlertDialogContent({
  ...
  // Drawer와 같은 이유로 폰 프레임 안에 포탈한다 — 기본 포탈 대상은 `document.body`라
  // 프레임의 `[contain:layout]`이 닿지 않고, 데스크톱에서 402px 액자를 벗어나 브라우저
  // 전체를 덮는다 (drawer.tsx의 같은 처리 참고).
  const frameRef = React.useContext(PhoneFrameContext)

  return (
    <AlertDialogPortal container={frameRef ?? undefined}>
      <AlertDialogOverlay />
