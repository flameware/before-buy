  ...
  return (
    <div
      ...
      className={
        ...
        // `--primary`는 밝은 라임이라 흰 배경에서 저알파는 거의 안 보인다 — 5/10/15/20을 띄워놓고
        // **눈으로** 고른 값이고, 계산으로는 안 나온다. 앰버 시절에 고른 10%를 라임에서도
        // 화면으로 확인하고 그대로 뒀다 — 밝기는 사실상 같고(L 0.987→0.986) 채도만 3할
        // 높아(C 0.0102→0.0133) 아주 조금 진해졌는데, 그 정도가 나았다.
        // 되돌릴 일이 생기면 8%가 앰버 시절의 채도와 거의 같다(C 0.0106).
    ...
    <>
      ...
      <div className="flex flex-1 flex-col gap-8 overflow-y-auto py-4">
        <section className="flex flex-col">
          ...
          {/* 전체폭 outline 버튼이 아니라 **내용폭 브랜드색 필**이다 — S5 `생각 업데이트 하기`와
