// **이미 실행이 끝난 스크립트다. 다시 돌리지 않는다 (#152).** 아래 값들은 #65 시점의
// 사본이고, #152에서 seed-data.ts의 임계값·문구가 다시 바뀌었으므로 지금 재실행하면
// 기존 세션에 **옛 값**을 덮어쓴다. #152는 백필하지 않기로 했다 — 새 방문자는 새 세션이라
// 새 상수로 프로비저닝되고, 영향받는 건 쿠키가 남은 개발자 본인뿐이라 시크릿 창이면 된다.
//
