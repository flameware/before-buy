...
Accepted. 단 두 대목이 뒤에 대체됐다 (Superseded in part by 0004, 0010).

- **"목록은 토글과 무관하게 유지된다"는 ADR-0004가 대체한다** — 데모 시점은 시세 표시
  옵션이 아니라 전제 판정의 입력이었고, 그 오해가 실제 버그로 드러났다.
- **"무효화는 데이터를 실제로 바꾸는 액션에서만 명시적으로 수행한다"는 ADR-0010이
  대체한다** — 무효화하는 그 시점에 S1은 언마운트 상태라 활성 옵저버가 없고, 그래서
  무효화는 stale 표시만 하고 refetch를 시작하지 않았다. 이제는 바꾼 액션의 응답을
  캐시에 옮겨 담고 무효화는 배경 재검증으로 강등한다 (#107).

React Query 도입과 목록/시세 분리라는 결론 자체는 유효하다.
...
- 무효화는 데이터를 실제로 바꾸는 액션에서만 명시적으로 수행한다: S1.5에서 종목 추가 시
  ...
  **(ADR-0010이 대체 — 무효화가 아니라 액션의 응답을 캐시에 옮겨 담는다.)**
