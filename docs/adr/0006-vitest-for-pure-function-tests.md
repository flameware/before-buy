...
- **순수 함수 경계는 한 번 넓혔다** — 브라우저 전용 인메모리 스토어(`lib/mock/thesis-draft.ts`)의
  상태 전이가 테스트 대상에 들어왔다(#122). 이 함수들은 모듈 스코프 `Map`을 읽고 쓰므로 순수하지
  않지만, 여기서 잠그는 것은 "draft 하나가 유상 호출을 최대 한 번 만든다"는 규칙이고 그 규칙이
  깨지면 나는 비용은 사람 눈에 안 띈다 — 화면은 멀쩡히 결과를 보여주고 세션 상한만 줄어든다.
  판정 기준은 순수함이 아니라 **서버 전용 모듈 없이 node에서 임포트되는가**이며, 스토어는 그
  조건을 지킨다(결과 타입만 `import type`으로 빌린다). 상태를 들고 있으므로 테스트는
  `beforeEach`로 스스로 자리를 비운다.
